...
const filteredBusinesses = orderedTitles.map(title => businesses.find(b => b.title === title)).filter(Boolean);
    ...
    <section id="businesses" className={styles.cardsSection}>
      ...
      <div className={styles.cardsGrid} id="cardsGrid">
        {visibleBusinesses.map((b, i) => {
          ...
          const CardTag = b.link ? 'a' : 'article';
          const linkProps = b.link ? { href: b.link, target: "_blank", rel: "noopener noreferrer", style: { textDecoration: 'none', display: 'block' } } : {};
          return (
            <CardTag 
              key={b.title} 
